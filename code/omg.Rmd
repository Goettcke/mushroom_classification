...
Splitting the data into a training data-set and a test set, is done because all the tested algorithms, can work with this partitioning, the cross-validation implementations in R might not have support for all of the tested algorithms, but this is of course also an option. The split results in a roughly 2/3 majority of negative labled instances in the test data, but as this is dichotomous classification and the correct prediction of either is seen as a correct classification, hence this is not seen as a bias.   
...
As can be seen here from the confusion matrix, there are only few true positives, when predicting if the mushrooms are edible. Which means we can't rely on our classification. The classification problem illustrates the necessity of making precise classifiers, cause here we're only 72.98% certain that the prediction of an edible mushroom is not poisonous. The precision is calculated tp/(tp+fp), so the amount of true positives i.e. edible mushroms predicted as edible, out of total amount of mushrooms predicted as edible. 
...
With a split at 80% training data and 20% test, the model performs impeccable i.e perfectly. The random forest only takes the number of trees as argument. The number of trees were incremented until a satisfying result were achieved, as can be seen in the barplot.  
...
```
The tree here gives a bit of insigt to how a tree the randomForrest looks like. With this small number of trees used to achieve perfect classification it is possible to get insigt into how the randomForrest makes decisions. 
...
```
![Caption](eval_summary.PNG)


##Conclusion 
Overall in this mini-study, the clear winner is the *randomForest*. There are many other methods, that could perform equivalently well, but it's interesting that Naïve Bayes performs so poorly in comparison with knn and randomForest. In this mushroom datasets, it was shown, that odor is a very important feature 